    ...
    for( std::size_t k = 0 ; k < 5 ; ++k )
    {
      for( std::size_t l = 0 ; l < 5 ; ++l )
      {
        for( std::size_t m = 0 ; m < 5 ; ++m )
        {
          ...
          //
          auto const i = iel * 125 + 8 * IDX3( m, l, k );

          //auto dxidx = _mm256_load_ps( &(rg_hexa_gll_dxidx[ 4176 * ( m + 5 * l + k * 25 ) + iel ]) );
          //auto detdx = _mm256_load_ps( &(rg_hexa_gll_detdx[ 4176 * ( m + 5 * l + k * 25 ) + iel ]) );
          //auto dzedx = _mm256_load_ps( &(rg_hexa_gll_dzedx[ 4176 * ( m + 5 * l + k * 25 ) + iel ]) );
          auto dxidx = _mm256_load_ps( &(rg_hexa_gll_dxidx[ i ]) );
          auto detdx = _mm256_load_ps( &(rg_hexa_gll_detdx[ i ]) );
          auto dzedx = _mm256_load_ps( &(rg_hexa_gll_dzedx[ i ]) );
          ...
          auto dxidy = _mm256_load_ps( &(rg_hexa_gll_dxidy[ i ]) );
          auto detdy = _mm256_load_ps( &(rg_hexa_gll_detdy[ i ]) );
          auto dzedy = _mm256_load_ps( &(rg_hexa_gll_dzedy[ i ]) );
          ...
          auto dxidz = _mm256_load_ps( &(rg_hexa_gll_dxidz[ i ]) );
          auto detdz = _mm256_load_ps( &(rg_hexa_gll_detdz[ i ]) );
          auto dzedz = _mm256_load_ps( &(rg_hexa_gll_dzedz[ i ]) );
          ...
          auto rhovp2 = _mm256_load_ps( &(rg_hexa_gll_rhovp2[ i ]) );
          auto rhovs2 = _mm256_load_ps( &(rg_hexa_gll_rhovs2[ i ]) );


          auto trace_tau = ( rhovp2 - _mm256_set1_ps( 2.0f ) * rhovs2 )*(duxdx+duydy+duzdz);
          auto tauxx     = trace_tau + _mm256_set1_ps( 2.0 )*rhovs2*duxdx;
          auto tauyy     = trace_tau + _mm256_set1_ps( 2.0 )*rhovs2*duydy;
          auto tauzz     = trace_tau + _mm256_set1_ps( 2.0 )*rhovs2*duzdz;
          auto tauxy     =                 rhovs2*(duxdy+duydx);
          auto tauxz     =                 rhovs2*(duxdz+duzdx);
          auto tauyz     =                 rhovs2*(duydz+duzdy);


          auto tmp = _mm256_load_ps( &(rg_hexa_gll_jacobian_det[ i ]) );
    ...
    for( std::size_t k = 0 ; k < 5 ; ++k )
    {
      for( std::size_t l = 0 ; l < 5 ; ++l )
      {
        for( std::size_t m = 0 ; m < 5 ; ++m )
        {
          ...
          float tx[ 8 ] __attribute((aligned(32)));//alignas(32);
          float ty[ 8 ] __attribute((aligned(32)));//alignas(32);
          float tz[ 8 ] __attribute((aligned(32)));//alignas(32);
